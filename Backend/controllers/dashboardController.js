...
//         const last60DaysIncomeTransactions = await Income.find({userId, date:{$gte: new Date(Date.now() - 60 * 60 * 24 * 1000)}}).sort({date:-1});
     
//        // get total income for last 60 days
       
//        const incomeLast60Days = last60DaysIncomeTransactions.reduce((sum, transaction) => sum + transaction.amount, 0);

//        const last30DaysExpenseTransactions = await Expense.find({userId, date:{$gte: new Date(Date.now() - 30 * 24 * 60 * 1000)},}).sort({date:-1});

//        const expenseLast30Days = last30DaysExpenseTransactions.reduce((sum, transaction) => sum + transaction.amount, 0);

//        // fetch last 5 transactions (income+expense)
        
//        const last5Transactions = [ ...(await Income.find({userId}).sort({date:-1}).limit(5)).map( (txn)=> ({...txn.toObject(), type: "income"}), ...(await Expense.find({userId}).sort({date:-1}).limit(5)).map( (txn)=> ({...txn.toObject(), type: "expense"}) )) ].sort((a,b) => b.date - a.date);

//        res.json({totalBalance:(totalIncome[0]?.total || 0) - (totalExpense[0]?.total || 0), totalIncome: totalIncome[0]?.total || 0, totalExpense: totalExpense[0]?.total || 0, last30DaysExpenses: {total: expenseLast30Days, transactions: last30DaysExpenseTransactions}, last60DaysIncome: {total: incomeLast60Days, transactions: last60DaysIncomeTransactions}, 
    
//       recentTransactions: lastTransactions,});
//     }catch(err){
//         res.status(500).json({message: "error getting dashboard data", err});
//     }
// }
...
exports.getDashboardData = async (req, res) => {
    try {
        ...
        console.error(err); // optional to debug
